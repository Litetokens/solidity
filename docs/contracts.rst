...
::
    ...
    contract Register is priced, owned {
        ...
        // automatically reject all Xlt sent to it.
...
Xlt (without data). Additionally, in order to receive Xlt, the fallback function
must be marked ``payable``. If no such function exists, the contract cannot receive
Xlt through regular transactions.
...
.. warning::
    Contracts that receive Xlt directly (without a function call, i.e. using ``send`` or ``transfer``)
    but do not define a fallback function
    throw an exception, sending back the Xlt (this was different
    before Solidity v0.4.0). So if you want your contract to receive Xlt,
    you have to implement a fallback function.

.. warning::
    A contract without a payable fallback function can receive Xlt as a recipient of a `coinbase transaction` (aka `miner block reward`)
    or as a destination of a ``selfdestruct``.

    A contract cannot react to such Xlt transfers and thus also cannot reject them. This is a design choice of the EVM and Solidity cannot work around it.
...
::
    ...
    contract Test {
        ...
        // Sending Xlt to this contract will cause an exception,
    ...
    // This contract keeps all Xlt sent to it with no way
    // to get it back.
    contract Sink {
    ...
    contract Caller {
        function callTest(Test test) public {
            ...
            // if someone sends xlt to that contract,
