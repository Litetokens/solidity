...
(seq
  ...
  ;; --------------------------------------------------------------------------
  ;; Will revert if sent any Xlt. We use the macro immediately so as
  ;;   to abort if sent any Xlt during contract deployment.
...
BOOST_AUTO_TEST_CASE(transfer_from)
{
	...
	// Send account(1) some xlt for gas.
	sendEther(account(1), 1000 * xlt);
	BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * xlt);
