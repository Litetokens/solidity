...
of Xlt hands over control to that contract (B). This makes it possible for B
...
::
    ...
    contract Fund {
        /// Mapping of xlt shares of the contract.
...
of ``send``, but it still exposes a weakness: Xlt transfer can always
include code execution, so the recipient could be a contract that calls
back into ``withdraw``. This would let it get multiple refunds and
basically retrieve all the Xlt in the contract. In particular, the
...
Note that re-entrancy is not only an effect of Xlt transfer but of any
...
Sending and Receiving Xlt
===========================

- Neither contracts nor "external accounts" are currently able to prevent that someone sends them Xlt.
  Contracts can react on and reject a regular transfer, but there are ways
  to move Xlt without creating a message call. One way is to simply "mine to"
  the contract address and the second way is using ``selfdestruct(x)``.

- If a contract receives Xlt (without a function being called), the fallback function is executed.
  If it does not have a fallback function, the Xlt will be rejected (by throwing an exception).
  During the execution of the fallback function, the contract can only rely
  on the "gas stipend" (2300 gas) being available to it at that time. This stipend is not enough to access storage in any way.
  To be sure that your contract can receive Xlt in that way, check the gas requirements of the fallback function
...
- If you want to send Xlt using ``address.transfer``, there are certain details to be aware of:
  ...
  2. Sending Xlt can fail due to the call depth going above 1024. Since the caller is in total control of the call
     ...
     write your contract using a pattern where the recipient can withdraw Xlt instead.
  3. Sending Xlt can also fail because the execution of the recipient contract
...
Now someone tricks you into sending xlt to the address of this attack wallet:
...
Restrict the Amount of Xlt
============================

Restrict the amount of Xlt (or other tokens) that can be stored in a smart
contract. If your source code, the compiler or the platform has a bug, these
funds may be lost. If you want to limit your loss, limit the amount of Xlt.
...
are the arguments in range, did they send enough Xlt, does the person
...
self-checks like "Has any Xlt leaked?",
