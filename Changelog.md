 ...
 * Introduce ``.transfer(value)`` for sending Xlt.
...
to explicitly specify if functions can receive xlt via the ``payable``
modifier. Furthermore, more situations cause exceptions to be thrown.

Minimal changes to be made for upgrade:
 - Add ``payable`` to all functions that want to receive Xlt
 ...
 * Functions that want to receive Xlt have to specify the
   new ``payable`` modifier (otherwise they throw).
 * Contracts that want to receive Xlt with a plain "send"
