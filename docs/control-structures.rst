...
::
    ...
    contract C {
        ...
        function createAndEndowD(uint arg, uint amount) public payable {
            // Send xlt along with the creation
            D newD = (new D).value(amount)(arg);
        }
    }

As seen in the example, it is possible to forward Xlt while creating
...
#. If your contract receives Xlt via a public function without ``payable`` modifier (including the constructor and the fallback function).
#. If your contract receives Xlt via a public getter function.
#. If a ``.transfer()`` fails.
...
::
    ...
    contract VendingMachine {
        function buy(uint amount) payable {
            if (amount > msg.value / 2 xlt)
                revert("Not enough Xlt provided.");
            // Alternative way to do it:
            require(
                amount <= msg.value / 2 xlt,
                "Not enough Xlt provided."
            );
...
In the above example, ``revert("Not enough Xlt provided.");`` will cause the following hexadecimal data be
